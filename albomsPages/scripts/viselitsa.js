import { viselitsa } from './../../JS/data/albums/viselitsa.js';

let currentTrackItem = null; // Переменная для хранения текущей активной карточки

function displayTracks(trackList) {
    const tracksContainer = document.querySelector('#viselitsaTracks');
...
const lyrics = [
    "Метеоритный дождь, падаю вниз (Метеоритный дождь)",
    "М.Л.Д., я в этом аду (М.Л.Д.)",
    "Сучка, ты знаешь, кто я (Сучка)",
    "Вверх-вниз, как на карусели (Вверх-вниз)",
    "Лучше, чем интернет, ты для меня (Лучше чем интернет)"
];

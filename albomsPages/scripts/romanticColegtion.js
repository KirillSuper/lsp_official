import { romanticColegtion } from './../../JS/data/albums/romanticColegtion.js';

let currentTrackItem = null; // Переменная для хранения текущей активной карточки

function displayTracks(trackList) {
    const tracksContainer = document.querySelector('#romanticColegtionTracks');
...
const lyrics = [
    "Любовь на расстоянии, но ты так близко (Любовь на расстоянии)",
    "Танцы в темноте, мы с тобой одни (Танцы в темноте)",
    "Сердце в огне, горит для тебя (Сердце в огне)",
    "Звёзды над нами, сияют для нас (Звёзды над нами)"
];
...
function init() {
    displayTracks(romanticColegtion);
